...
<!-- si es admin el titulo es mensajes con XUSUARIO y los mensjaes del admin estaran eb ka derech -->
    ...
    <div id="head">
        <h2>{{ __('language.admin-mess')}}</h2>
        <a href="{{route('chatUser',[Auth::user()->id])}}">
            <span class="iconify" data-icon="ph:arrows-counter-clockwise" style="color: #b33061;" data-width="30" data-height="30"></span>
        </a>
    </div>
    {{-- end head --}}
    <!-- se sacan todos los mensajes del chat  y se comprueba si son de admin o no poniendolo en diferentes collumnas-->
    <div id="conver">
        @foreach ($sms as $smss)
            @if($smss->isAdmin==1)
